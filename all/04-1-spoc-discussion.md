...
2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
   ...
   均衡繁忙应该是说整个系统的各方面虽然都不是理想情况，但是都是可接受的，即成本可以接受，访问速度可以接受，存储空间可以接受。访问快的层次多访问，访问慢的层次少访问。

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

   就地存储的方法，比如矩阵LLt分解可以使用原矩阵存放运算结果。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

   在单个程序内存不够用时，将程序各部分重叠放置。工作是将程序分块，计算互相依赖性，在程序执行时将使用的块和闲置的块做交换。

2. 什么是交换技术？覆盖与交换有什么不同？

   交换技术是将空闲的进程交换到外存中，增加活动进程可使用空间的技术。覆盖技术的视角是单个程序，交换的视角是所有程序的调度。
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   时间局部性是同一条指令或同一个数据在短时间内会被多次访问。

   空间局部性是相邻的指令或数据会被连续访问。

   分支局部性是同一个分支跳转语句在大部分情况下会有相同的跳转结果。

2. 如何提高程序执行时的局部性特征？

   在编程时注意根据局部性原理编写循环的代码，增加连续访问的可能性。
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   ...
   虚拟存储是将程序的一部分移到外存中。它使得应用程序编写相对容易，交换更为自由，挑战是需要处理缺页异常。
...
### 8.5 虚拟页式存储
 ...
 1. 为了支持虚拟页式存储的实现，页表项有什么修改？
    ...
    增加了标志位，用于记录当前页的信息，如驻留，保护，修改，访问。


 2. 页式存储和虚拟页式存储的区别是什么？

    页式存储是直接对物理内存进行分页存储，物理内存的大小决定了页的数量。

    虚拟页式存储是对虚拟内存进行分页，与物理内存的大小无关，但是更为复杂，需要处理更多问题。

### 8.6 缺页异常

1. 缺页异常的处理流程？

   操作系统在外存中找到对应的页，在内存中找到空闲的页（若无空闲页，则根据算法替换出空闲页），将外存中的页调入内存，修改页表中对应的项，接着访问。


2. 虚拟页式存储管理中有效存储访问时间是如何计算的？

   T = p\*内存访问时间 + (1-p)\*外存访问时间。
...
双异常出现于中断服务例程不在内存中的情况。正常情况下内核部分一定放在内存中所以不应该出现这个问题。如果真的出现了，系统此时不能再触发中断，否则会导致系统shutdown。
...
对齐的情况下2次，一次是指令带来的缺页，一次是数据带来的缺页；

不对齐的情况下最多4次，指令和数据都跨页。
